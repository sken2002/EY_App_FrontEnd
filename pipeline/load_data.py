...
def load_all():
    ...
    dq_issues = pd.read_csv(os.path.join(DATA_DIR, 'DataQualityIssues.csv'))
    data_inventory = pd.read_csv(os.path.join(DATA_DIR, 'DataInventory.csv'))
    ...
    # DataQualityIssues dates
    for c in ['DetectedDate', 'ResolvedDate', 'LastUpdated']:
        if c in dq_issues.columns:
            dq_issues[c] = pd.to_datetime(dq_issues[c], errors='coerce')

    # DataInventory dates
    for c in ['LastUpdated', 'CreatedDate', 'LastAudited']:
        if c in data_inventory.columns:
            data_inventory[c] = pd.to_datetime(data_inventory[c], errors='coerce')
    ...
    print(f"Loaded: WP={len(wp)}, MS={len(ms)}, SO={len(so)}, Contracts={len(contracts)}, "
          ...
          f"DQIssues={len(dq_issues)}, DataInventory={len(data_inventory)}")

    return {
        'wp': wp, 'ms': ms, 'so': so, 'contracts': contracts,
        'scores': scores, 'cost_perf': cost_perf, 'cashflow': cashflow, 'org': org,
        'dq_issues': dq_issues, 'data_inventory': data_inventory
    }
